...
\t\t errmsg("integer out of range")));""".format(
      c_check
    )
  if intermediate_type:
    body += "\nresult = result2;"
...
def write_sortsupport_c_function(
  f,
  typ,
  pgversion
):
...
def write_arithmetic_op(
  ...
  if op == '*':
    if type_bits(rettype) < 64:
      intermediate_type = next_bigger_type(
        rettype
      )
      c_check = '({0}) result2 != result2'.format(
        c_types[rettype]
      )
    elif type_unsigned(rettype):
      if type_unsigned(leftarg) and type_signed(
        rightarg
      ):
        c_check = '(arg2 < 0) || ('
      elif type_signed(leftarg) and type_unsigned(
        rightarg
      ):
        c_check = '(arg1 < 0) || ('
      else:
      ...
      if type_unsigned(leftarg) or type_unsigned(
        rightarg
      ):
...
""".format(
  ...
  if op in ['/', '%']:
    f_test_sql.write(
      "SELECT '5'::{lefttype} {op} '0'::{righttype};\n"
      .format(
        lefttype=leftarg,
        op=op,
        righttype=rightarg
  ...
  if op in ['%']:
    f_test_sql.write(
      "SELECT mod('5'::{lefttype}, '2'::{righttype});\n"
      .format(
        lefttype=leftarg,
        righttype=rightarg
      )
...
""".format(
        ...
        vals=(
          range(1,
                6) if type_unsigned(argtype) else
          range(-2,
                3)
        )
      )
...
""".format(lefttype=leftarg,
      ...
      if leftarg != rightarg:
        ...
        body = "result = arg1;"
        if type_bits(leftarg
                     ) >= type_bits(rightarg):
          body += """
if (({c_type}) result != arg1)
\tereport(ERROR,
\t\t(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
\t\t errmsg("{typ} out of range")));""".format(
            c_type=c_types[leftarg],
            typ=rightarg
          )
        if type_unsigned(
          leftarg
        ) != type_unsigned(rightarg):
          body += """
...
\t\t errmsg("{typ} out of range")));""".format(
        ...
        f_sql.write(
          ...
          .format(
            ...
            arg=leftarg,
            context=(
              "IMPLICIT" if type_bits(leftarg) <
              type_bits(rightarg) else
              "ASSIGNMENT"
            )
          )
        )
...
""".format(sfunc=sfunc,
    ...
    sfunc = "{argtype}_avg_accum".format(
      argtype=arg
    )
